...
Adapt exercise for weather (indoor stretching/yoga if rainy/poor AQI). Respect allergies in meals."""
    ...
    if client is None:
        return _fallback_plan(user_profile, city_report)
...
def _fallback_plan(user_profile: dict, city_report: dict) -> dict[str, Any]:
    ...
    is_pakistan = city.lower() in ["quetta", "karachi", "lahore", "islamabad", "peshawar", "rawalpindi", "multan", "quetta override"] or "pakistan" in country.lower()

    if is_pakistan or city.lower() == "quetta":
        breakfast = "Boiled eggs, whole-wheat roti/paratha, and a small bowl of chanay (chickpeas)"
        if allergies and "egg" in allergies.lower():
            breakfast = "Whole-wheat roti with chanay (chickpeas) and light green tea (Kahwah)"

        lunch = "Chicken Pulao or Mutton Rosh with a fresh mixed salad"
        dinner = "Slow-cooked Namkeen Rosh or Grilled Sajji with whole-wheat roti and mint raita"
        snacks = "A fresh local apple or pomegranate, accompanied by green tea (Kahwah)"
        exercise_activity = f"30-min brisk walk in a local park or trail" if outdoor else "30-min indoor home stretching and breathing exercises"
    else:
        breakfast = "Oatmeal with berries and almonds"
        if allergies and "nut" in allergies.lower():
            breakfast = "Oatmeal with berries and sunflower seeds"
        lunch = "Grilled chicken salad with quinoa"
        dinner = "Baked salmon with steamed vegetables"
        snacks = "Greek yogurt with honey"
        exercise_activity = f"30-min brisk walk in {city}" if outdoor else "30-min indoor yoga and stretching"

    exercise = {"time": "7:00 AM", "activity": exercise_activity, "duration_minutes": 30, "indoor": not outdoor}

    return {
        "wake_up": {"time": "6:30 AM", "notes": "Start with 2 glasses of warm water"},
        "exercise": exercise,
        "breakfast": {"time": "8:00 AM", "meal": breakfast, "calories_approx": 380},
        "lunch": {"time": "12:30 PM", "meal": lunch},
        "dinner": {"time": "7:00 PM", "meal": dinner},
        "snacks": {"time": "3:30 PM", "meal": snacks},
        "hydration": {"target_litres": 2.5, "reminders": ["10:00 AM", "2:00 PM", "5:00 PM"]},
        "relaxation": {"time": "9:00 PM", "activity": "10-minute guided breathing meditation or deep breathing (pranayama)"},
        "sleep": {"time": "10:30 PM", "target_hours": 8},
        "health_tip": f"Today's air quality in {city} favours {'outdoor' if outdoor else 'indoor'} activities.",
    }
...
def run_planner_agent(user_profile: dict, city_report: dict) -> dict[str, Any]:
    ...
    if llm is not None:
        ...
        try:
            ...
            logger.warning("CrewAI planner failed, using direct LLM: %s", exc)
...
Disclaimer to append: {MEDICAL_DISCLAIMER}"""
    ...
    if client is None:
        return (
            ...
            f"Based on general health guidelines: {rag_context[:500]}...\n\n"
    ...
    try:
        ...
        return f"I apologise, I encountered an error. Please try again.\n\n{MEDICAL_DISCLAIMER}"
